...
class MineScreen extends ConsumerWidget {
          ...
          TextButton(
            onPressed: () async {
              ...
              try {
                ...
                // Bug#F001 fix: 将CSV写入临时文件并分享，而不是只复制到剪贴板
                final dir = await getTemporaryDirectory();
                final dateStr = DateTime.now().toIso8601String().slice(0, 10);
                final file = File('${dir.path}/小牛记账_$dateStr.csv');
                // BOM + UTF-8 避免Excel打开乱码
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              // Bug#F002 fix: 在进入 async 前同步捕获 authState，避免 ref 在 async 回调中失效崩溃
              final authState = ref.read(authProvider);
              Navigator.pop(ctx);
              try {
                // 先读取文件内容
                final filePath = controller.text.trim();
                final fileContent = await _readFile(filePath, authState);
                // 调用导入API
                final api = ref.read(apiProvider);
  ...
  // Bug#F002 fix: authState 通过参数传入，避免在 async 函数中使用失效的 ref
  Future<String> _readFile(String path, AuthState authState) async {
    final phone = authState.phone ?? '';
    final token = authState.token ?? '';
    final uri = Uri.parse('${DEFAULT_BASE_URL}/api/file/read').replace(
      queryParameters: {'path': path},
    );
    final res = await http.get(uri, headers: {'x-phone': phone, 'x-token': token});
