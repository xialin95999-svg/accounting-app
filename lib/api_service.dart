...
class ApiService {
  ...
  /// 保存预算配置
  Future<Map> saveBudget(Map budget) async {
    final res = await http.put(
      Uri.parse('$baseUrl/api/budget'),
      headers: _headers,
  ...
  /// 获取资产统计数据（总资产/总负债/净资产）
  Future<Map> getAssets() async {
    final res = await http.get(
  ...
  /// 设置账户初始余额（用于计算资产）
  Future<Map> setAccountInitialBalance(String accountId, double initialBalance) async {
    final res = await http.put(
      Uri.parse('$baseUrl/api/accounts/$accountId/balance'),
      headers: _headers,
      body: jsonEncode({'initial_balance': initialBalance}),
    );
    return _handle(res);
  }
  ...
  /// 获取年度统计数据
  Future<Map> getYearlyStats({int? year}) async {
    final params = <String, String>{};
    if (year != null) params['year'] = year.toString();
    final uri = Uri.parse('$baseUrl/api/stats/yearly').replace(queryParameters: params);
    final res = await http.get(uri, headers: _headers);
    return _handle(res);
  }
